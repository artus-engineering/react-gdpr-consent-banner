...
export function CookieConsentBanner(): JSX.Element {
  ...
  function getCookieConsentState(): CookieConsentState {
    return cookieProviders.reduce((acc, cookie) => {
      ...
      acc[cookie.category].cookies[cookie.id] =
        cookie.category === "Essential" || getCookieSelection(cookie);
      acc[cookie.category].enabled = Object.values(
        acc[cookie.category].cookies
      ).every((value) => value);
      return acc;
  ...
  function setSelectedCookies() {
    cookieProviders.forEach((provider) =>
      persistCookieSelection(
        provider,
        getSelectionState(provider),
        config.domain,
        config.cookiesValidForDays
      )
    );
  }
  ...
  function handleCategoryToggle(category: string) {
    ...
    setConsentState((prevState) => {
      ...
      const newCookiesState = Object.keys(
  ...
  function handleCookieToggle(category: string, cookieId: string) {
    ...
    setConsentState((prevState) => {
      const newCookieState =
        !prevState[category as CookieCategory].cookies[cookieId];
      const newCookiesState = {
        ...prevState[category as CookieCategory].cookies,
        [cookieId]: newCookieState,
      };
      const newEnabledState = Object.values(newCookiesState).every(
        (value) => value
      );
      return {
        ...prevState,
  ...
  function getCategoriesWithCookies(): [string, { enabled: boolean; cookies: { [cookieId: string]: boolean } }][] {
    return Object.entries(consentState).filter(
      ([, categoryState]) => Object.keys(categoryState.cookies).length > 0
    );
  }
      ...
      >
        <div style={{ padding: "20px 24px", overflowY: "auto", flex: 1 }}>
          <div style={{ marginBottom: "20px" }}>
            ...
            <p style={{ color: style.textSecondary, fontSize: "13px", marginTop: "4px", marginBottom: 0 }}>
              {config.lang === "deDE"
                ? "Ihre Zustimmung gilt für: "
                : "Your consent applies to: "}
              <span style={{ fontWeight: 600, color: style.textPrimary }}>
                {getDomainsText()}
              </span>
            </p>
          </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
          {getCategoriesWithCookies().map(([category, categoryState]) => {
            const categoryProviders = cookieProviders.filter(
                              ...
                              >
                                ...
                                <div style={{ display: "flex", flexDirection: "column", gap: "3px" }}>
                                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: "10px" }}>
                                    <span style={{ color: style.textSecondary, fontWeight: 500 }}>
                                      {getLabel("details", "cookieName", config)}
                                    </span>
                                    <span style={{ color: style.textPrimary, fontFamily: "monospace" }}>
                                      {cookie.name}
                                    </span>
                                  </div>
                                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: "10px" }}>
                                    ...
                                    <span style={{ color: style.textPrimary }}>
                                      {cookie.duration}{" "}
                                      {getUnit(cookie.duration, cookie.unit, config)}
                                    </span>
                                  </div>
                                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: "10px" }}>
                                    <span style={{ color: style.textSecondary, fontWeight: 500 }}>
                                    ...
                                    <span style={{ color: style.textPrimary, textAlign: "right", maxWidth: "60%" }}>
